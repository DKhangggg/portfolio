...
// --- DỮ LIỆU cardData VIẾT CỨNG ĐÃ BỊ XÓA KHỎI ĐÂY ---
...
const BentoCardGrid: React.FC<{
  ...
  <div
    // Sửa max-w ở đây nếu bạn muốn nó rộng hơn (ví dụ: max-w-6xl)
    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {/* --- BẮT ĐẦU LOGIC MAP ĐÃ SỬA --- */}
          {cardData.map((card, index) => {
            // 1. ĐỊNH NGHĨA CÁC THUỘC TÍNH CHUNG
            ...
            // 2. ĐỊNH NGHĨA NỘI DUNG BÊN TRONG CARD (ĐỂ TÁI SỬ DỤNG)
            ...
            // 3. TẠO COMPONENT CARD (CÓ PARTICLE HOẶC KHÔNG)
            let cardComponent;
            ...
            if (enableStars) {
              ...
              cardComponent = (
                <div
                  ...
                  ref={(el) => {
                    if (!el) return;
                    // ... (Code xử lý event mouse của bạn ở đây) ...
                  }}
                >
                  {cardInnerContent}
                </div>
            ...
            // 4. KIỂM TRA HREF VÀ BỌC BẰNG <a> NẾU CẦN
            if (card.href) {
              return (
            ...
            // 5. NẾU KHÔNG CÓ HREF, RETURN CARD (PHẢI THÊM KEY VÀO ĐÂY)
            // (Chúng ta dùng React.cloneElement để thêm 'key' vào component đã tạo)
            return React.cloneElement(cardComponent, { key: index });
          })}
          {/* --- KẾT THÚC LOGIC MAP ĐÃ SỬA --- */}
        </div>
      </BentoCardGrid>
    </>
  );
